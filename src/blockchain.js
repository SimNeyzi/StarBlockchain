...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const validateArr = await self.validateChain()
                if (validateArr.length === 0) {
                    self.chain.push(block);
                    self.height = self.chain.length;
                    resolve(block);
                } else {
                    reject ('Error happened when validating block in _addBlock')
                }
    ...
    submitStar(address, message, signature, star) {
        let self = this;
        return new Promise(async (resolve, reject) => {
            let msgSentTime = parseInt(message.split(':')[1]);
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
            self.chain.filter(block => block.hash === hash)[0];
        });
    }
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (chain.length > 1) {
                for (let i = 1; i < chain.length; i++) {
                    ...
                    let isValid = await current.validate()
                    if (!(isValid && previous.hash === current.previousBlockHash)) {
